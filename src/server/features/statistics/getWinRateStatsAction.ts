...
import { getWinRateStats } from './StatisticsRepository';

type Params = {
  filters: unknown;
};

export async function getWinRateStatsAction({ filters }: Params) {
  try {
    ...
    const result = await getWinRateStats(validationResult.data);
    ...
    console.error('[getWinRateStatsAction]', error);
    return {
      success: false,
